...
def parse_args():
    ...
    parser.add_argument('--fix_occur', type=utils.str2bool, default=False, help='whether to train occurrence matrix')
    ...
    parser.add_argument("--start_epoch", type=int, default=10, help='start to decay lr')
...
def eval_model(args, model, loss_func, val_data, epoch):
    model.eval()
    val_loss = 0
    val_num = 0
...
def train(args, model_id, tb, train_data, val_data):
    ...
    model.train()
    optimizer = utils.build_optimizer(args, model.parameters())
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_data):
            ...
            if iteration % args.val_every == 0:
                val_loss = eval_model(args, model, loss_func, val_data, epoch)
